...
export class ListAttributeComponent {
  ...
  listAttributes(page = 1){
    this.attributeService.listAttributes(page,this.search).subscribe((resp:any) => {
      console.log(resp);
  ...
  openModalDeletePropertie(propertie: any) {
    ...
    modalRef.componentInstance.PropertieD.subscribe((resp: any) => {
        const index = this.attributes.findIndex((item: any) => item.id === propertie.id);
        if (index !== -1) {
            this.attributes.splice(index, 1);
        }
    });
